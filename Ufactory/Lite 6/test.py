# S/N : XYZARIS0V3P2402N04
# Robot IP : '192.168.1.177'
# script version : v3.2.0.1
...
class RobotMain(object):
    ...
    def __init__(self, robot, **kwargs):
        ...
        self.position_jig_A_grab = [-255.6, -138.6, 196, -18.1, 89.3, -135.1] #linear
        self.position_jig_B_grab = [-152.3, -129.0, 196, 4.8, 89.0, -90.7] #linear
        self.position_jig_C_grab = [-77.8, -148.6, 196, 4.0, 88.9, -51.8] #linear
        self.position_sealing_check = [-136.8, 71.5, 307.6, 39.7, -82.8, -28.5] #Linear
        self.position_capsule_place = [237, 135, 461.4, -143.8, 89.4, -57.3] #Linear
        self.position_before_capsule_place = self.position_capsule_place.copy()
        self.position_before_capsule_place[2] += 25
        self.position_cup_grab = [215.6, -100.2, 144.1, -75.8, -89.6, 146.3] #linear
        #before icecream
        self.position_topping_A = [-200.3, 162.8, 345, -31.7, 87.8, 96.1] #Linear
        self.position_topping_B = [106.5, -39.7, 15.0, 158.7, 40.4, 16.9] #Angle
        self.position_topping_C = [15, 137.9, 340, -92.8, 87.5, 5.3] #Linear
        ...
        self.position_jig_C_serve = [-63.1, -138.2, 206, -45.5, 88.1, -112.1] #Linear
        self.position_capsule_grab = [239.5, 135.0, 463.5, -140.1, 88.0, -52.5] #Linear
    ...
    def motion_grab_cup(self):
        ...
        if not self._check_code(code, 'set_position'):
            return
        code = self._arm.set_servo_angle(angle=[-16.2, -33.2, 47.4, 64.7, 18.6, 44.8], speed=self._angle_speed, mvacc=self._angle_acc, wait=False, radius=20.0)
        if not self._check_code(code, 'set_servo_angle'):
            return
        code = self._arm.open_lite6_gripper()
        if not self._check_code(code, 'open_lite6_gripper'):
